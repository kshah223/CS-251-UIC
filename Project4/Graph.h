  ...
  public:
    ...
    */
    ...
    bool valid_topo_order(const vector<int> & order) {
		...
		vector<bool> vec = vector<bool>(num_nodes());
		...
		while(i < order.size()) {
			int j = order[i];
			int count = vertices[j].incoming.size();
			
			if(count > 0) {
				int x = 0;
				while(x < count) {
					if(vec[vertices[j].incoming[x].vertex_id] == false)
						return false;
					x++;
				}
			}
    ...
    */
    ...
    bool enum_paths(int target, vector<string> &paths) {
		...
		for(int i = 0; i < num_nodes(); i++) {
			int s = vec[i];
			if(vertices[s].incoming.size() == 0) {
				_enum_paths(target,paths,vec[i],id2name(s));
			}
		}
	...
	void _enum_paths(int target, vector<string> &paths, int &x, string path_so_far) {
		int v = 0;
		for(edge &e : vertices[x].outgoing) {
			v = e.vertex_id;
			if(v == target) {
				string t = path_so_far + " " + id2name(v);
				paths.push_back(t);
				return;
			}
			else {
				string t = path_so_far + " " + id2name(v);
				_enum_paths(target,paths,v,t);
			}
		}		
	}
    /*

   ...
   */
    ...
    */
    bool is_sorted()const {
        ...
        for(Node*l1 = front; l1 != nullptr && l1->next != nullptr; l1 = l1->next) {
            if(compare(l1,l1->next) == worse) {
                return false;
            }
        }
        return true;
    ...
    */
    bool is_pareto() const {
        ...
        for(Node*l1 = front; l1 != nullptr; l1=l1->next) {
            for(Node*l2 = l1->next; l2 != nullptr; l2=l2->next) {
                Relationship status = compare(l1->price,l1->time,l2->price,l2->time);
    ...
    */
    bool is_pareto_sorted() const {
		...
		for(Node*l1 = front; l1 != nullptr && l1->next != nullptr; l1= l1->next) {
			if((l1->next->price <= l1->price) && (l1->next->time >= l1->time)) {
				return false;
			}
		}
		return true;
    ...
    bool insert_sorted(double price, double time) {
		...
		while (curr != nullptr && (curr->price < price || (curr->price == price && curr->time < time)))
		{
			prev = curr;
			curr = curr->next;
		}
    ...
    bool insert_pareto_sorted(double price, double time) {
		if (!is_pareto_sorted()) 
			...
			while(l1 != nullptr) {
				...
				if(compare(l1->price,l1->time,price,time) == worse) {
					Node* tmp = l1;
					l1 = l1->next;
					if (prev != nullptr) {
						prev->next = l1;
				...
				else if(compare(l1->price,l1->time,price,time) == better || compare(l1->price,l1->time,price,time) == equal) {
					deleted = false;
					break;
				}
				else {
    ...
    */
    ...
    TravelOptions* union_pareto_sorted(const TravelOptions& other)const {
		...
		for(Node* curr = front; curr != nullptr; curr = curr->next) {
			t->insert_pareto_sorted(curr->price,curr->time);
		}
		for(Node* curr1 = other.front; curr1 != nullptr; curr1 = curr1->next) {
			t->insert_pareto_sorted(curr1->price,curr1->time);
		}
    ...
    */
    bool prune_sorted() {
		...
		while(l1 != nullptr && l1->next != nullptr) {
			if(compare(l1,l1->next) != incomparable && compare(l1,l1->next) != worse) {
				Node* tmp = l1;
				l1 = l1->next;
				if (prev1 != nullptr) {
					prev1->next = l1;
